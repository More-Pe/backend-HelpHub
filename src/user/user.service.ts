...
export class UserService {
    ...
    try {
      ...
      if (userExists.length == 1) {
        ...
        const userCreate = await this.userModel.create({
          ...
          surnameUser: createUserDto.surnameUser,
          phone: createUserDto.phone,
          showPhone: createUserDto.showPhone,
          optionCall: createUserDto.optionCall,
  ...
  async update(id: string, updateUserDto: UpdateUserDto) {
    try {
      const { nameUser, surnameUser, phone, optionCall, showPhone } =
        updateUserDto;
