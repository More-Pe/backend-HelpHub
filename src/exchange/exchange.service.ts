...
export class ExchangeService {
  ...
  async createExchange(
    ...
    try {
      ...
      const existingChange = await this.exchangeModel.find({
        transmitter: transmitter,
        reciever: reciever,
      });
      const existingChange2 = await this.exchangeModel.find({
  ...
  async findAllApp(id: string) {
    try {
      const exchanges1 = await this.exchangeModel.find({
        reciever: id,
        state: 'accepted',
      });
      const exchanges2 = await this.exchangeModel.find({
        transmitter: id,
        state: 'accepted',
      });
  ...
  async updateExchange(updateExchangeDto: UpdateExchangeDto, id: string) {
    try {
      ...
      if (!exchangeValid) {
        ...
        const exchangeUpdate = await this.exchangeModel.findByIdAndUpdate(id, {
          state: state,
        });
